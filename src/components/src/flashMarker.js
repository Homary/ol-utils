...
Marker.prototype.draw = function(context, map) {
	context.save();
	context.beginPath();
	switch (this.type) {
		case 'circle':
			this._drawCircle(context, map);
			break;
		case 'ellipse':
			this._drawEllipse(context, map);
			break;
		default:
			break;
	}
...
Marker.prototype._drawCircle = function(context, map) {
...
Marker.prototype._drawEllipse = function(context, map) {
	try {
		var pixel = this.pixel || map.getPixelFromCoordinate(this.location);
	} catch (e) {
		console.log(map.getPixelFromCoordinate(this.location));
	}
	try {
		var x = pixel[0],
			y = pixel[1],
